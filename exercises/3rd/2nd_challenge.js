 ...
 * sum of its elements using a different algorithm than the third
 ...
 * @param length An integer numeric value (optional)
 */
function getSumOfAllIn(array, length) {

    // Defining the base case of the recursion.
    if (length == 0) {
        return 0;
    }

    // Defining the "length" param if it do not exists. 
    if (!length) {
        length = array.length;
    }

    // Defining the recursion algorithm
    return getSumOfAllIn(array, length - 1) + array[length - 1];
